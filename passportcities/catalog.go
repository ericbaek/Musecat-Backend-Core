...
func ReadGeoNames(r io.Reader, accept func(City) error) error {
	return readGeoNames(r, false, accept)
}

func readGeoNames(r io.Reader, sections bool, accept func(City) error) error {
	...
	for line := 1; scanner.Scan(); line++ {
		...
		code := fields[7]
		if fields[6] != "P" || !(code == "PPL" || code == "PPLC" || code == "PPLA" || code == "PPLA2" || code == "PPLA3" || code == "PPLA4" || code == "PPLA5" || (sections && code == "PPLX")) {
			continue
		}
		id, e1 := strconv.ParseInt(fields[0], 10, 64)
		lat, e2 := strconv.ParseFloat(fields[4], 64)
		lon, e3 := strconv.ParseFloat(fields[5], 64)
		if e1 != nil || id < 1 || e2 != nil || e3 != nil || !(lat >= -90 && lat <= 90 && lon >= -180 && lon <= 180) || len(fields[8]) != 2 {
			return fmt.Errorf("invalid GeoNames city at line %d", line)
		}
		// A missing administrative identifier cannot support unambiguous matching.
		// Official extracts contain these rows; skip them without aborting import.
		if fields[10] == "" {
			continue
		}
		c := City{FeatureCode: code, SourceID: fields[0], Name: fields[1], Country: fields[8], Admin1: fields[10], Aliases: append([]string{fields[2]}, strings.Split(fields[3], ",")...), Lat: lat, Lon: lon}
