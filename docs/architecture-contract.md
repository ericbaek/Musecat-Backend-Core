...
2. `arcade` is the aggregate root. A molecule is a versioned relation owned by that aggregate; an atom is owned by its molecule or upload flow. Changelog rows are immutable audit history.
...
| `arcade_basic`, `hour`, `sns`, `gtk`, `game`, `photo` | versioned molecule for one aggregate section | corresponding `handlers/arcade/<part>` handler | A replacement molecule is created, then the root pointer changes in the same transaction. |
