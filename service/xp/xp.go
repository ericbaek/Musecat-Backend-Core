...
const (
	CollectionUser         = "user"
	CollectionUserLevel    = "user_level"
	CollectionUserLevelLog = "user_level_log"
...
func ExpToNextLevel(exp int) (int, int) {
	level := LevelFromExp(exp)
	nextExp := NextLevelExp(level)
	return level, nextExp - exp
}
...
func ParseLevelLogCreated(created string) (time.Time, error) {
	return parseLevelLogCreated(created)
}
...
func LoadAttendanceExp(app core.App, userID string) (int, error) {
	return loadAttendanceExp(app, userID)
}
